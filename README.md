...
```text
# fields: earliest and latest trace step seen, filepath, first and last line numbers, average calls (calls / distinct lines)
...
```text
# fields: filepath, line number✖calls|...
...
* `--src <DIR>`: Verifies trace log executions against valid Python files inside a targeted source tree root. Disambiguates duplicate file basenames, converts outputs to relative source paths, and rejects trace entries outside line count bounds.
